...
if __name__ == '__main__':
  ...
  #filename = 'rancat_1623434_2.2000052_3.3998463_WITHsyscuts.fits'
   filename = 'rancat_1623434_2.2000052_3.3998463_WITHsyscuts_RADECzs.fits'
 ...
 # for k in range(1343,int(nn)):
   ...
   if doJob:
	...
	col4 = fits.Column(name='pi', format='D', array=pi[w])
	col5 = fits.Column(name='rp', format='D', array=rp[w])
	col6 = fits.Column(name='s', format='D', array=ss[w])

	cols = fits.ColDefs([col1, col2, col3, col4, col5, col6])
	tbhdu = fits.BinTableHDU.from_columns(cols)
	
	# clobber = True  (it overwrites/updates)
	tbhdu.writeto('RR_withsyscus/python.out.'+str(int(k))+'.100_d_pi_rp_s.fits', clobber=True)
	...
	hist = np.histogram(d[w], bins = bins)
        hist_s = np.histogram(ss[w], bins = edges)
	...
	tbhdu = fits.BinTableHDU.from_columns(cols)
	tbhdu.writeto('RR_withsyscus_hist/python.out.'+str(int(k))+'.100.hist_d_s_NGC_SGC.fits', clobber=True)
	### END - Calculating South
