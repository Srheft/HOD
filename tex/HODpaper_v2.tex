...
To remove the added assumptions for including other measurements in the fit, we fit the same two-parameter HOD model used in prior fitting processes only to our single bin of measured $w_p$ from the KDE-complete sample of close pairs and the Mpc-scale measurement from eBOSS. The solid blue line and its shaded envelope in Figure \,\ref{fig6:kdeboss} represent the best-fit to this data (KDE+eBOSS) and its 1-$\sigma$ uncertainty. The solid dark red model and its associated envelope is the best-fit model to the same measurements but also the measured $w_p$ over eight bins of comoving distance from KO12. The shaded envelopes that are representing the 1-$\sigma$ uncertainties for the best fit model, are visibly separated, making it clear that inclusion of the clustering measurement at $z\sim1.4$ results in a different best-fit model (see the second and the last row of Table\,\ref{tab:res}).
