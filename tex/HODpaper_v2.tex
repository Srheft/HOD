...
To remove the added assumptions for including past measurements in the results, we fit the same two-parameter HOD model used in prior fitting processes only to our measured $w_p$ from the KDE-complete sample of close pairs and our Mpc-scale measurement from the eBOSS sample, leaving KO12 measurement out of the fitting procedure. The solid blue line in Figure \,\ref{fig6:kdeboss} represents the best-fit to this data (KDE+eBOSS). The solid dark red model is the best-fit model to the same KDE+eBOSS measurements but also the measured $w_p$ over eight bins of comoving distance from KO12. The shaded envelopes that represent the 1-$\sigma$ uncertainties for the best-fit models, are visibly separated. This indicates that inclusion of the clustering measurement at $z\sim1.4$ results in a significantly different best-fit (see the second and the last row of Table\,\ref{tab:res}). The lower clustering signal measured for quasars in KO12 sample results in $> \times 30$ smaller satellite fraction compared to what KDE clustering alone implies. 
...
\hline
 ...
 \vspace{.1in}
 1KDE+eBOSS & $2.32_{-0.46}^{+0.44}$& $0.062_{-0.028}^{+0.026}$& 0.855 (12)\\
