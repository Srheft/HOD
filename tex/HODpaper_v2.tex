...
best fits the observational data. The solid blue curve is the bestfit and the red dashed and green dotted dashed lines are the model with best fit $M_{m}$ but $f_{sat}=0.01$ and $f_{sat}=1.0$. We verified that relaxing $\Delta_{\rm m}$, as the third parameter, has no 
significant effect on the shape of the best-fit model. The best fit value for $\Delta_{\rm m}$ stays cosistent with 0.75 that was derived independently by \citet{ko12}. The shaded envelope 
around the best fit model is the extent of the 2-sigma certainty for the shape 
of the HOD model at each bin of the projected distance ($\rm r_p$). All the data points depicted here have participated in the fit. See 
...
0.016_{-0.0053}^{+0.0043}$ with reduced $\chi^{2} = 1.1$. Right: The result of the 2-parameter Monte-Carlo fit to the projected 
two point correlation function over $ \sim 0.01-100 \, \rm h^{-1} Mpc$ for the model shown in the left. Diagonal 
...
f_{sat} = 1.0$  Right: Same as the right panel for the fitting process shown in the right-hand side of Fig.\,\ref{fig3:bfit}.}\label{fig4:cor}
       \end{subfigure}
\end{figure*}
...
% best fits the full-scale measurement (where all the shown data points participated in the best fit determination). The dashed blue line is the 
...
best fit model as shown in the left-hand panel of Fig.\ref{fig4:bfit}. The orange dashed and the cyan dotted dashed lines are the HOD model with the best-fit $\rm f_{sat}$ and $\rm M_{m}$ but with 10 and 20 times larger concentration parameters than the best-fit model, respectively. }\label{fig5:10c}
    \end{subfigure}
\end{figure*}
