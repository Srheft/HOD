...
% 
\author[Eftekharzadeh et al.]{S. Eftekharzadeh$^{1,2}$,  A. D. Myers$^{2}$, E. Kourkchi$^{3}$, et al.\\
\\
...
satellite fractions using their two different HOD parametrizations. Much of this degeneracy regarding the form of the quasar 
...
\citep{my15,daw16} and surveys with the {\em Dark Energy Spectropic Instrument} \citep{DESI1,DESI2} are beginning to use 
...
parametrization is described in $\S$\ref{hodmod}, and 
...
Given the considerations discussed in \S\ref{dat}, the final samples became suitable for covering the full range of comoving separations for measuring the real-space correlation function of quasars at $z\sim 1.55$.  Here we summarize the steps taken in clustering measurements at kpc and Mpc-scales.


\subsection{kpc-scale clustering}\label{kpcls}
The method for measuring the volume-averaged correlation function in real space with sparse samples of close pairs is discussed in detail in \citet{ef17} and \citet{Hen06}. Fig. 2 of \citet{ef17} shows how the sample of 47 pairs are positioned in the redshit-physical separation plane. We apply the redshift cut of $0.8 \lesssim z \lesssim 2.2$ and count the remaining pairs in linearly shifted bins of separation (i.e., to the comving coordinates). This constitutes the origin of the closely separated bins in the measured correlation function shown in Fig.\ref{fig3:}
...
\hline
\end{tabular}
\caption{ Normalized distribution of the spectroscopic redshifts of quasars in eBOSS (4th column) and the KDE-complete sample of close pairs (6th column).}\label{tab:dndz}
...
\begin{figure*}
    ...
    \begin{subfigure}{
     \centering
       \includegraphics[angle=0,scale=0.6]{Bestfit_bestfit_wp10c_2pars_4KDE.eps}
...
best fits the full-scale measurement (where all the shown data points participated in the bestfit determination). The solid blue line is the 
...
parameters. Right: Same as the left panel for the fitting process shwon in the right-hand side of Fig.\,\ref{fig3:bfit}.}\label{fig5:cor}
       \end{subfigure}
\end{figure*}
...
between the best-fit parameters of widely used HOD formalisms (initially 
