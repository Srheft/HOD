...
\end{equation}
where $n(z)$ is the comoving number density of quasara at bins of redshift, $v_z \equiv v_{max}(1+z)/H(z)$, $v_{max}= 2000 \rm s^{-1}km$, $H(z)$ is the expansion rate at redshift z, $N_{qso} \sim \int_{z_{min}}^{z_{max}} dz \frac{dV_c}{dz} n(z)$ and $\int_{0}^{v_z} d\pi \xi(\sqrt{\left(r_p^2+\pi^2 \right)},z)$ is essentially $w_p(r_p, v_z \rightarrow \infty)$. \\

Similar approximation with different numerical approach has been used in \citet{ko12} for $ \bar W_p(r_p) \rightarrow w_p(r_p)$ conversion.
